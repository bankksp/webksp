...
import { 
  ...
  Calendar, Info, Layout, Download,
  ExternalLink, Eye, Check
} from 'lucide-react';
...
export const InfoDocumentManager = () => {
  ...
  const handleOpenModal = (doc?: InfoDocument) => {
      if (doc) {
        ...
        setFormData({
          ...
          thumbnailUrl: doc.thumbnailUrl || '',
          description: doc.description || '',
          year: doc.year || String(new Date().getFullYear() + 543)
        });
      } else {
        setEditingDoc(null);
        setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    const toastId = toast.loading(editingDoc ? 'กำลังอัปเดตข้อมูล...' : 'กำลังบันทึกข้อมูล...');
    try {
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900">จัดการสารสนเทศ/เอกสาร</h1>
          <p className="text-gray-500">จัดการเอกสาร PDF และสารสนเทศต่างๆ ของโรงเรียน</p>
        </div>
        <button 
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      <a 
                        ...
                        className="w-10 h-14 rounded-lg overflow-hidden flex-shrink-0 bg-gray-100 border border-gray-200 hover:opacity-80 transition-opacity"
                      >
                        <img 
                      ...
                      <div className="min-w-0">
                        <a 
                          ...
                          target="_blank" 
                          rel="noopener noreferrer"
                          className="font-bold text-gray-900 truncate max-w-xs hover:text-indigo-600 transition-colors block"
                        >
                          {doc.title}
                        </a>
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2 md:col-span-2 p-6 bg-red-50/30 border border-red-100 rounded-3xl">
                    <label className="text-sm font-bold text-red-700 ml-1 flex items-center gap-2">
                      <FileText size={16} /> ไฟล์เอกสาร (PDF) <span className="text-[10px] bg-red-100 px-2 py-0.5 rounded-full uppercase tracking-widest">Required</span>
                    </label>
                    <FileUpload 
                      currentImageUrl={formData.pdfUrl}
                      onUploadSuccess={(url) => setFormData({...formData, pdfUrl: url})}
                      accept=".pdf,application/pdf"
                      label="คลิกเพื่ออัปโหลดไฟล์ PDF"
                    />
                  </div>

                  <div className="space-y-2 md:col-span-2 p-6 bg-indigo-50/30 border border-indigo-100 rounded-3xl">
                    <label className="text-sm font-bold text-indigo-700 ml-1 flex items-center gap-2">
                      <Layout size={16} /> รูปหน้าปก (Thumbnail) <span className="text-[10px] bg-indigo-100 px-2 py-0.5 rounded-full uppercase tracking-widest">Optional</span>
                    </label>
                    <FileUpload 
